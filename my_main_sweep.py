...
def train_audiocaps(config, checkpoint_dir=None):
    ...
    for epoch in range(alg_config["epochs"] + 1):
        ...
        checkpoint_dir_ = config['output_path'] + '/audiocaps_checkpoints/checkpoint_%06d' % epoch
...
def train(checkpoint_dir=None):
    ...
    with wandb.init(config=config) as run:
        ...
        for epoch in range(alg_config["epochs"] + 1):
            ...
            model_utils.train(model, optimizer, criterion, text_loaders["train"], epoch, use_sentence_embeddings, use_auto_caption_embeddings, log_wandb)
            # print(model.audio_encoder.pointwise_conv.weight.data)

            epoch_results = {}
            for split in ["train", "val", "test"]:
                epoch_results["{0}_loss".format(split)] = model_utils.eval(model, criterion, text_loaders[split], use_sentence_embeddings, use_auto_caption_embeddings)
            ...
            checkpoint_dir_ = config['output_path'] + '/checkpoints_seed_%d_%s_%.3f/checkpoint_%06d'%(torch_seed, audioset_info, ratio_embed1, epoch)
            if not os.path.exists(checkpoint_dir_):
                os.makedirs(checkpoint_dir_)
...
if __name__ == "__main__":
    ...
    # if log_wandb:
        ...
        # wandb.init(project="lbar22", entity="topel", name="__passt_mpnet_seed%d_lr%.3f_epochs%d_bs%d_mixup%d_%s_%.3f_margin_%.3f"%(torch_seed,

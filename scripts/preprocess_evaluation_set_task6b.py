...
# # for the 2022 Task 6b official eval subset
global_params = {
    "dataset_dir": "/baie/corpus/DCASE2022/evaluation_set_task6b",
    "audio_splits": ["test"],
    "caption_files": ["new_retrieval_captions.csv"]
}
...
for split, caption_file in zip(global_params["audio_splits"], global_params["caption_files"]):
    captions = pd.read_csv(os.path.join(global_params["dataset_dir"], caption_file), delimiter='\t')
    # print(captions)
    ...
    for i in captions.index:
        ...
        for cap in [c1]:
            ...
            print(split, cid_count, fid_count, cap)

            text, words = word_utils.clean_text(cap)

            # cid, fid, fname, original, caption, tokens
            audio_captions.append([cid_count, fid_count, fname, cap, text, words])
            # print(audio_captions[-1])
